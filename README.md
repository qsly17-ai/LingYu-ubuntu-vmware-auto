...
`-RootPassword` 始终是构建模式必需参数，因为 Packer 第一次 SSH 连接需要用它登录临时 bootstrap 用户。`-RootPasswordHash` 只用于高级场景：覆盖最终 root 账号写入的 SHA-512 crypt 哈希，不能单独替代 `-RootPassword`。

如果 `-RootPassword` 不是 `root`，脚本需要 `openssl.exe` 来生成临时 bootstrap 用户的 SHA-512 crypt 哈希。安装过 Git for Windows 的机器通常已经带有 OpenSSL；只做本地测试时可以直接使用 `-RootPassword root`。

默认构建完成后会删除生成的 Packer 变量文件和 cloud-init 文件，避免明文 SSH 密码长期留在 `build` 目录。需要排查构建问题时，可以加 `-KeepBuildSecrets` 保留这些文件。

默认会移除临时 bootstrap 用户的免密 sudo 配置，但保留该用户可登录，作为 root 登录异常时的兜底入口。确认 root 登录无误并希望构建后锁定 bootstrap 用户时，可以加 `-LockBootstrapUser`。
...
这是第三方源。脚本会在安装前校验 Authenticode 签名，只接受签名发布者包含 `VMware` 或 `Broadcom` 的安装包。使用默认地址时，脚本还会自动校验下面这个 SHA256：

```text
a0ef9087607d9cad20b08139e73e41242e044ad5bd8cee141d3bad314586737f
```

如果覆盖 `-VmwareInstallerUrl` 或使用 `-VmwareInstallerPath`，建议同时传入 `-VmwareInstallerSha256` 做额外哈希校验。
...
- `-RootPasswordHash`：构建 VM 时覆盖最终 root 账号的 SHA-512 crypt 哈希；不能单独替代 `-RootPassword`。
- `-ForceRebuild`：删除并重建已有虚拟机输出目录。
- `-ShowConsole`：构建时显示 VMware 窗口。
- `-LockBootstrapUser`：Packer 构建完成后锁定临时 bootstrap 用户。
- `-KeepBuildSecrets`：保留生成的 Packer 变量文件和 cloud-init 文件，方便排查构建问题。
...
不要把弱密码用于公网或不可信网络。默认准备模式不会写入任何 VM 密码，只有用户显式 `-BuildVm -RootPassword` 时才会把密码写入临时构建配置。脚本默认会在 Packer 结束后删除这些临时配置；如果使用 `-KeepBuildSecrets`，请在排查完成后手动清理 `build` 目录。
